...
class TA2Particle : public TObject
{
 ...
 public:
  ...
  virtual ~TA2Particle();
  // NOTE Added for MWPC
  void SetIintersMwpc(const Int_t i0, const Int_t i1) { fIintersMwpc[0] = i0; fIintersMwpc[1] = i1; }
  void SetIinterMwpc(const Int_t iCh, const Int_t i) { fIintersMwpc[iCh] = i; }
  void SetEtrackMwpc(const Double_t &e) { fEtrackMwpc = e; }
  void SetTtrackMwpc(const Double_t &t) { fTtrackMwpc = t; }
  const Int_t    &GetIinterMwpc(const Int_t iCh) const { return fIintersMwpc[iCh]; }
  const Int_t    *GetIintersMwpc() const { return fIintersMwpc; }
  const Double_t &GetEtrackMwpc() const { return fEtrackMwpc; }
  const Double_t &GetTtrackMwpc() const { return fTtrackMwpc; }
  //
